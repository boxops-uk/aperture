...
# index-repo.sh puts into Aperture.
#
...
#           half that corresponds to an Aperture run's walk *and* its write, minus the
#           interning: no fact has an id yet.
#   load  — `glean write`, which parses each batch, interns every nested fact bottom-up
#           and assigns the ids. This is the half that corresponds to Aperture's server.
#
# The honest total for this side is emit + load. Saying it as one number would hide that
# Glean's pipeline has a file in the middle and Aperture's does not, which is a real
...
#   GLEAN_DIR     scratch root (default: /tmp/ap-glean)
set -euo pipefail
...
scratch="${GLEAN_DIR:-/tmp/ap-glean}"
...
# core count, and the Aperture side of the comparison is given --jobs 8 for the same
# reason.
#
...
# an Aperture database that has been through `aperture finish`.
load_start=$(date +%s)
...
echo "      'apbench.SearchByName { name = \"Parse\" }' --stats -"
