...
/// <c>int</c> in <c>schemas/code.aps</c> is a <c>nat</c> in <c>apbench.angle</c>. Every
...
internal static class GleanFacts
{
    ...
    /// identical — <c>apbench.Decl</c> is <c>src.Decl</c> — and so are the field orders,
    /// which on both systems are the key's byte order and therefore the index design.
    /// </remarks>
    public const string Namespace = "apbench";

    /// <summary>The version every predicate in that schema carries.</summary>
    public const int Version = 1;

    /// <summary>The namespace the Aperture side states its predicates in.</summary>
    private const string ApertureNamespace = "src.";
    ...
    /// <summary>
    /// <c>src.Decl</c> becomes <c>apbench.Decl.1</c>.
    /// </summary>
    /// <remarks>
    /// Derived rather than tabulated. A second hand-written name table would be a second
    /// thing to keep in step with <c>apbench.angle</c>, and the translation it would
    /// state is mechanical — the schema was written to preserve every name.
    /// </remarks>
    public static string PredicateName(string apertureName) =>
        apertureName.StartsWith(ApertureNamespace, StringComparison.Ordinal)
            ? $"{Namespace}.{apertureName[ApertureNamespace.Length..]}.{Version}"
            : throw new InvalidOperationException(
                $"`{apertureName}` is not in the `{ApertureNamespace}` schema, so there is "
    ...
    public static void WriteBatch(
        ...
        uint predicate,
        IReadOnlyList<ApertureFact> facts)
    {
    ...
    /// </remarks>
    private static void WriteFact(Utf8JsonWriter json, ApertureSchema schema, ApertureFact fact)
    {
        ...
        ApertureValue value)
    {
        switch (type, value)
        {
            case (ApertureType.Int, ApertureValue.Int number):
                if (number.Value < 0)
                {
                    throw new InvalidOperationException(
                        $"{number.Value} cannot be written as a Glean `nat`; this schema's "
                        + "`int` fields are positions and lengths, and a negative one means "
                        + "the corpus is not the one apbench.angle was translated for");
                }

                json.WriteNumberValue(number.Value);
                break;

            case (ApertureType.Str, ApertureValue.Str text):
                json.WriteStringValue(Utf8Safe(text.Value));
                break;

            case (ApertureType.Fact declared, ApertureValue.Ref reference):
            ...
            case (ApertureType.Record declared, ApertureValue.Record record):
            {
                ...
                // Named here, positional on the Aperture wire — the same field order
        ...
        ApertureRef reference)
    {
        switch (reference)
        {
            ...
            case ApertureRef.Id id:
                ...
                // file is an input to. An Aperture id is none of those — its top bits
                ...
                throw new InvalidOperationException(
                    $"reference to Aperture fact id {id.FactId} cannot be written as a Glean "
    ...
    /// is enough. <c>Encoding.UTF8.GetBytes</c>, which the Aperture codec uses,
...
/// </para>
/// </remarks>
internal sealed class GleanTarget(ApertureSchema schema, string directory, int writer) : IBlockTarget
{
    ...
    public BlockWritten Write(uint predicate, IReadOnlyList<ApertureFact> facts)
    {
