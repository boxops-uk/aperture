...
/// about Aperture. Pointing the same walk at a different fact database is then one
...
/// Aperture write stream carries one predicate per <c>CopyData</c> frame, and a Glean
...
internal interface IBlockTarget : IDisposable
{
    ...
    BlockWritten Write(uint predicate, IReadOnlyList<ApertureFact> facts);
}
...
/// target that does not know — the Aperture client encodes inside itself and does not
/// report a size.
/// </remarks>
internal readonly record struct BlockWritten(ulong Created, ulong Deduped, long Bytes);

/// <summary>A block written down one Aperture connection.</summary>
/// <remarks>
...
/// </remarks>
internal sealed class ApertureTarget(ApertureConnection connection) : IBlockTarget
{
