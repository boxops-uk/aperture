...
///   <item><term><c>/run/aperture.sock//code</c></term><description>a Unix socket</description></item>
...
/// </para>
/// </remarks>
public sealed record ApertureAddress
{
    ...
    private ApertureAddress(string? socketPath, string? host, int port, string database)
    {
    ...
    public static ApertureAddress Parse(string text)
    {
        ...
        if (where.Length == 0)
        {
            return new ApertureAddress(null, null, 0, database);
        }

        if (IsPath(where))
        {
            return new ApertureAddress(ExpandHome(where), null, 0, database);
        }
        ...
        return new ApertureAddress(null, host, port, database);
    }

    /// <summary>An address at <paramref name="socketPath"/>, for a caller that holds one.</summary>
    public static ApertureAddress ForSocket(string socketPath, string database) =>
        new(socketPath, null, 0, database);

    /// <summary>This address with a socket filled in if it named no target.</summary>
    public ApertureAddress OrSocket(string socketPath) =>
        HasTarget ? this : new ApertureAddress(socketPath, null, 0, Database);
