...
//! [`WireFact`](fjord_wire::WireFact) — schema-driven, references possibly
//! *nested* — and leaves as rows in two column families, keyed by the
//! order-preserving [storage codec](fjord_encoding::tuple) and identified by a
//! [`FactId`](fjord_schema::id::FactId). Three crates meet to make that happen and
...
//! implementation:
//!
//! - **Dedup** is [`intern`](fjord_store::store::Interned)'s "the id already
