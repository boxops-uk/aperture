...
fn schema() -> Schema {
    ...
    let cst = fjord_schema::syntax::parse::parse(SOURCE, &mut diagnostics).expect("it parses");
...
/// A path with a quote in it cannot end the sigla literal it lands in.
///
...
fn no_page_reads_a_predicate_whole() {
    ...
    for (name, query) in fjord_viewer::query::census() {
