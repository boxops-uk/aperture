...
//! # What it is not
//!
//! **It reaches nothing below [`fjord_client`].** No store, no engine, no plan: a
...
//! same reason `fjord_cli::workload` is one module — a query written where it is
//! used is a query nobody can cost.
//!
//! # Blocking client, async server
//!
//! [`fjord_client`] is synchronous by design. So every question runs on a blocking
...
async fn run<T: Send + 'static>(
    app: &Arc<App>,
    f: impl FnOnce(&mut fjord_client::Connection) -> Result<T, ClientError> + Send + 'static,
