...
//! [phase 11](../../../website/content/clients.md) is about which of these seek and
...
use fjord_client::{ClientError, Connection};
use fjord_wire::{Desc, WireRef, WireValue};

/// A row, **read by field name**.
///
/// Not by position, and that distinction cost an afternoon: a query's head record has
/// its fields **sorted by name at lowering** (`AGENTS.md`), so
...
/// Escape a string for a sigla literal.
///
...
pub fn literal(text: &str) -> String {
    ...
    for c in text.chars() {
        match c {
            '"' => out.push_str("\\\""),
            '\\' => out.push_str("\\\\"),
            // Control characters cannot appear in a sigla string at all, so they are
...
///
/// A prefix seek into `src.SearchByLowerName`, which exists because sigla has no
...
/// deliberate for the reason `fjord_engine::diag::Code::ALL` is hand-written:
