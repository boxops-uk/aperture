...
// trait Store {
//     fn fetch_entity(&self, id: FactId) -> Result<Entity, FjordError>;
// }
...
// impl FieldOffsets {
//     pub fn scan(key: &ByteView, max: FieldIdx) -> Result<Self, FjordError> {
...
//     fn field(&self, field_idx: FieldIdx, key: ByteView) -> Result<ByteView, FjordError> {
//         let range = self
//             .ranges
//             .get(field_idx.0 as usize)
//             .ok_or(FjordError::FieldIndexOutOfBounds(field_idx))?;
...
// impl<'s> Executor<'s> {
//     fn exec(&self, plan: &Plan) -> Result<u64, FjordError> {
...
//                 value[..size_of::<u32>()]
//                     .try_into()
//                     .map_err(|_| FjordError::MalformedKey)?,
//             );
...
//     pub fn decode_int(field: &[u8]) -> Result<i64, FjordError> {
//         let tag = *field.first().ok_or(FjordError::MalformedKey)?;
//         if tag == M_INT_ZERO {
//             return Ok(0);
//         }
//         let mut buf = [0u8; 8];
...
//             if field.len() != 1 + n {
//                 return Err(FjordError::MalformedKey);
//             }
...
//         } else {
//             Err(FjordError::UnexpectedMarker(tag))
//         }
//     }
...
//         field: &'a [u8],
//     ) -> Result<std::borrow::Cow<'a, [u8]>, FjordError> {
//         use std::borrow::Cow;
//         if field.len() < 2 {
//             return Err(FjordError::MalformedKey);
//         }
...
//     pub fn skip_terminated(src: &[u8], mut i: usize) -> Result<usize, FjordError> {
//         loop {
//             match src.get(i) {
//                 None => return Err(FjordError::MalformedKey),
...
//     pub fn skip_one(src: &[u8], i: usize, nested: bool) -> Result<usize, FjordError> {
//         let tag = *src.get(i).ok_or(FjordError::MalformedKey)?;
//         let after_tag = i + 1;
//         match tag {
//             M_NULL => {
//                 if nested {
...
//                 Ok(end)
//             }
//             other => Err(FjordError::UnexpectedMarker(other)),
//         }
//     }
...
//     pub fn decode_value(src: &[u8], nested: bool) -> Result<(Value, &[u8]), FjordError> {
...
//                 Value::Str(String::from_utf8(bytes).map_err(|_| FjordError::MalformedKey)?)
//             }
//             M_RECORD => {
...
//             other => return Err(FjordError::UnexpectedMarker(other)),
//         };
//         Ok((v, rest))
//     }
// }
