...
//! Fjord address is a Fjord address.
//!
...
///
/// Done here rather than left to fail at connect time: a quoted `'~/db/fjord.sock//code'`
/// reaches us with the tilde intact, and "no such file `~/db/fjord.sock`" is a worse
...
mod tests {
    ...
    fn an_absolute_path_is_a_socket() {
        assert_eq!(
            parse("/run/user/1000/fjord.sock//code").endpoint(),
            Some(&Endpoint::Unix(PathBuf::from("/run/user/1000/fjord.sock")))
        );
    }
    ...
    fn a_default_target_fills_in_only_where_none_was_named() {
        let fallback = Endpoint::Unix(PathBuf::from("/run/fjord.sock"));
