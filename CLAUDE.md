...
| root `aperture-cli` | the **tool**: `cli` (the clap tree, §4), `config` (where things live), `commands/` (create, list, describe, finish, rm, serve, query, **shell** — the wire REPL, `\more` holding a real cursor — plus `route`, §2's address resolution stated once over `aperture-client`), `output` (rendering, always client-side), `shell` (Phase 5's *embedded demo*, kept for `:plan`/`:type`, which need a compiler in-process), `code_index` — the built-in schema, *parsed* from `schemas/code.aps` since Phase 8.4 — and `workload`, the one statement of what the `examples/` instruments measure (Phase 10's S0). The binary is `aperture` |
