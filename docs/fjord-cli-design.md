# Fjord DB — CLI Design & Operational Requirements
...
against Fjord's **design**, and that asymmetry is worth keeping in view: Glean's costs are
measured and admitted in its own source, Fjord's are predicted. The ledger of what is
...
- **ops-I3 — Finish ordering.** `finish` makes all data durable first (fjall
  ...
  **Why Glean needs a third state and Fjord does not.** Glean's lifecycle is
  ...
  **Fjord does compact inside `finish`, and still needs no third state**, which is worth being
  ...
  testing-only. Fjord cannot offer it at all: a recorded content hash cannot survive an
...
- **ops-I4 — Reproducibility.** A DB built twice from identical inputs is identical.
  ...
  **The chain, written out — and its second arrow has been cut.** As first stated it read:
  ...
  (`crates/fjord-store/src/identity.rs`, whose module doc says outright that a `FactId` is
  ...
  **What Glean pays instead.** A Glean DB is not reproducible, for six independent reasons —
  ...
  Fjord's price wherever it wants determinism too: `glean merge` processes its input files
...
- **ops-I9 — No cross-DB anything in P0.** No cross-DB queries, no stacking, no ownership sets.
  **Two mechanisms, priced apart, because only one of them is expensive.** *Stacking* composes
  ...
  apparatus costs about **7%** of DB size. Fjord declines both halves, but only the second is
  a hard divergence — the full cost breakdown is in [the ledger](glean-comparison.md).
  **The seam actually kept is `FactStore::{scan, point}`** (`crates/fjord-engine/src/plan.rs`) — the direct
  ...
  `Access { predicate_id, seek_key }` (`crates/fjord-engine/src/plan.rs`) **is** that address, with no
  section/layer dimension, and `Cursor` (`crates/fjord-engine/src/iter.rs`) carries no layer tag: those two are
  ...
  (`crates/fjord-store/src/store.rs`) already derives each per-predicate boundary from the data rather than
  ...
  *for* incrementality with the sign flipped: its stated problems are indexing jobs "taking
  ...
  side of the comparison — **Fjord states no target corpus size, no churn rate and no
...
nothing needs to announce that a Fjord address is a Fjord address — `fjord://`
...
2. An address naming a target goes there, and **has no offline half**. §2's rule forbids
   ...
   named is worse. `FJORD_TARGET` and a config file's `target` count as naming one.
...
**Every client uses this grammar**, not only the CLI — `fjord-viewer` takes it
positionally, and the .NET indexer and demo take it as `--at`, with `FjordAddress`
...
Layered, .NET-style: **default → config file → env (`FJORD_` prefix) → CLI flag**, every
...
**The file is `./fjord.json`, or `--config <path>`. The working directory only, with no
...
| `data_dir` | server, offline lifecycle | store root. A **named** root keeps its socket beside it (`<root>/fjord.sock`); the default root uses `$XDG_RUNTIME_DIR/fjord.sock`, which is short on purpose — a socket path has a hard length limit and one derived from a deep data directory is a path the kernel refuses |
...
| `schema_path` | schema commands, `create` | ordered roots used to resolve a *named* schema to its entry file; also `FJORD_SCHEMA_PATH`, first-match-wins. `mod` edges within a schema resolve relative to the referencing file (§7), not via this path. |
...
- Exposes enumeration as the virtual predicate `fjord.db.List` through the normal query
...
- **A readiness signal.** `--write-port FILE` (Glean's
  ...
  server is up". Fjord's socket path is derived, not chosen, so the file only has to appear —
...
- Meta-commands mirroring psql: `\l` (list — issues `fjord.db.List`), `\d [pred]`
  ...
  **Amended as built: the commands are `:`-prefixed and the `\` spellings are aliases.** Both
  prefixes are accepted because neither can begin a sigla query, so a hand trained on psql costs
...
- **`:expand` — show the fact a reference names, recursively.** Not in the original list, and it
  ...
  — a number naming a fact the reader cannot see — and *sigla cannot ask*, since a query names a
  ...
  Four things fall out, and each is why this shape rather than another:
    ...
    - **A virtual predicate expands like anything else**, and that is the seam's promise kept
      rather than an exception made. `X where X = fjord.db.List _` heads on a whole catalogue
  ...
  `fjord query --expand[=hops]` is the same thing non-interactively, which is what makes a
  row a script can read without a second query to resolve each id.
- **Built, and `fjord.db.List` is answered at the `FactStore` seam** rather than by a new
...
- **`\profile` — facts searched per predicate, with a full-scan flag.** `\d`/`:plan` shows
  *intent* — which field narrowed the scan, which one only filters; this shows the *outcome*, and
  ...
  surfaced.
  **Built, and reachable before the shell is**: `fjord query --profile` prints the same table,
...
- Remote (address given): query `fjord.db.List`; output identical either way.
...
- **Per-predicate stats are a gap with real leverage, and cheap here.** The design lists counts
  ...
  on sizing derivation batches. Fjord's per-predicate keyspaces make a per-predicate count
  nearly free, and `finish` is the natural place to record it.

### `fjord db backup <db> <dest>` / `db restore <archive> [name]`
...
- **At-rest structural validation is a genuine gap, and the shape to copy exists.** A fingerprint
  ...
  things (`glean/rts/validate.cpp:109-145`), two of which are literally Fjord's engine
...
### `fjord schema fingerprint [ROOT|FILE...]`
...
- PG-inspired, **not** PG-compatible. Startup/handshake PG-shaped (version, DB name, session
  ...
  (`fjord-wire::frame`), 9 bytes. `kind` is a byte with named constants rather than a closed
...
- **`F`/`f` — "what facts do these ids name?"** A batch of ids out; one **key** each back, in the
  ...
  the way in: stored, a reference is a `FactId`, so a row carries a number, and sigla cannot ask
...
**Built** — `fjord-wire` (`crates/fjord-wire/src/`), whose module docs are the design of
record for the encoding itself. The shape of it, and what each choice is *not* paying for:

| | storage (`fjord-encoding`) | transport (`fjord-wire`) |
|---|---|---|
...
**Interning — built** (`fjord-ingest`), against a real store, with `FjallDb::intern` as the
...
- **A control frame is a stream like any other.** `create`, `finish` and `remove` are frames on an
  ...
  every database under the root, and §5's remote branch answers them through `fjord.db.List`.
...
- **Block — built** (`fjord-wire::block`), 30 bytes of framing:
...
- **This is a real advantage over Glean, and it is the splittability, not the encoding.** A Glean
  ...
  chunking decision onto the producer; Fjord splits *one file* at validated sync markers, so a
...
```
<data_dir>/
├── fjord.sock                     # (or per-config) server socket; presence ⇒ server detection
└── <name>/<instance>/             # instance: provisional ULID; identity fp lives in sidecar
    ├── FJORD_META                 # sidecar: atomic temp+fsync+rename writes (ops-I3)
...
├── crates/
│   ├── fjord-schema/              # parse → AST → canonical model; imports/resolution;
│   │                              # fingerprints; subset-containment compatibility
│   ├── fjord-encoding/            # order-preserving storage tuple codec; wire value codec;
│   │                              # reserved sync-marker constants
│   ├── fjord-store/               # Store trait: fjall backend + in-memory B-tree (tests);
...
│   ├── fjord-engine/              # existing query engine; resumable iteratee executor;
│   │                              # consumes (store handle, snapshot) — no server assumptions
│   ├── fjord-ingest/              # fact-file format (§8); sync-scan splitter;
│   │                              # parallel decode/sort; k-way merge w/ dedup + conflict-reject;
│   │                              # fjall bulk-ingest sink
│   ├── fjord-wire/                # frame codec, message types, stream state machine —
│   │                              # shared by server and client, no I/O policy
│   ├── fjord-client/              # connection, handshake, session modes, stream mux,
│   │                              # COPY writer, and a query result as a *bookmark*
│   │                              # (`take` is the page `\more` resumes) — used by
│   │                              # CLI, shell, and external tools/derivers
│   ├── fjord-server/              # listener, per-conn fair writer task, session enforcement
│   │                              # (ops-I6), per-DB seal barrier, fjord.db.List
│   └── fjord-cli/                 # bin
│       └── src/
...
`server` for the single reason that `fjord serve` is a subcommand — the tool is where the
